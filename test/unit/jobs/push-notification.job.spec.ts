...
import { FcmService } from '../../../src/infrastructure/notifications/fcm.service';
import { QueueService } from '../../../src/infrastructure/queue/queue.service';
import { PushNotificationJob } from '../../../src/jobs/push-notification.job';
...
describe('PushNotificationJob', () => {
  it('registers a queue handler and marks queued push deliveries as sent', async () => {
    ...
    const job = new PushNotificationJob(
      queueService,
    ...
    expect(notificationsService.markQueuedPushDeliveriesSent).toHaveBeenCalledWith(
      'notification_1',
      'provider_msg_1',
    );
  ...
  it('marks queued push deliveries as failed when no active push tokens exist', async () => {
    ...
    expect(notificationsService.markQueuedPushDeliveriesFailed).toHaveBeenCalledWith(
      'notification_1',
  ...
  it('removes invalid push tokens but still marks delivery sent when at least one device token succeeds', async () => {
    ...
    expect(notificationsService.deletePushTokensByIds).toHaveBeenCalledWith(
      'usr_merchant_1',
      ['push_2'],
    );
    expect(notificationsService.markQueuedPushDeliveriesSent).toHaveBeenCalledWith(
  ...
  it('schedules a retry with backoff when the provider fails transiently before max attempts', async () => {
    ...
    expect(notificationsService.markQueuedPushDeliveriesFailed).toHaveBeenCalledWith(
      ...
      'Attempt 1 of 3 failed. Retry 2 scheduled in 5000ms. FCM temporary outage',
    );
  ...
  it('marks delivery failed without retry when the provider fails on the final attempt', async () => {
    ...
    expect(notificationsService.markQueuedPushDeliveriesFailed).toHaveBeenCalledWith(
      'notification_1',
      'PUSH_PROVIDER_ERROR',
      'Attempt 3 of 3 failed. FCM temporary outage',
    );
