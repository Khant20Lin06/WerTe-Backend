...
import { DlqEntry, DlqService } from '../../../../src/infrastructure/queue/dlq.service';

// ioredis mocked — no real Redis connection needed.
const mockRedis = {
...
describe('DlqService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new DlqService(makeConfigService(), makeLogger());
  });
  ...
  describe('push()', () => {
    ...
    it('logs an error event with job details', async () => {
      ...
      service = new DlqService(makeConfigService(), logger);
      const entry = makeEntry();
