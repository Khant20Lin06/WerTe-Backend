...
import {
  BranchCatalogRecord,
  buildBranchCatalog,
} from '../../../../src/modules/menus/entities/branch-catalog.entity';

describe('buildBranchCatalog', () => {
  const makeBranchCatalogRecord = (
  ...
  it('keeps inactive catalog nodes when activeOnly is disabled', () => {
    const catalog = buildBranchCatalog(makeBranchCatalogRecord());
  ...
  it('filters inactive and unavailable nodes when activeOnly is enabled', () => {
    ...
    expect(catalog.categories[0].items[0].optionGroups[0].optionGroupId).toBe(
      'group_active',
    );
    expect(catalog.categories[0].items[0].optionGroups[0].kind).toBe(
      ItemOptionGroupKind.VARIANT_SELECTOR,
    );
  ...
  it('filters scoped categories and items for a selected store type', () => {
    ...
    expect(
      catalog.categories.find((category) => category.categoryId === 'cat_pharmacy')
        ?.scopedStoreTypes,
    ).toEqual([
      expect.objectContaining({
        code: 'pharmacy',
