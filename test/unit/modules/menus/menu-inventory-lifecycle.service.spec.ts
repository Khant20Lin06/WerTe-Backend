...
describe('MenuInventoryLifecycleService', () => {
  ...
  it('reserves tracked item and option inventory together', async () => {
    ...
    const service = new MenuInventoryLifecycleService(repository);
    ...
    expect(repository.decrementTrackedItemStock).toHaveBeenCalledWith(
      'item_1',
      2,
      undefined,
    );
    expect(repository.decrementTrackedOptionStock).toHaveBeenCalledWith(
      'option_1',
      2,
      undefined,
    );
  ...
  it('fails with conflict when a tracked option can no longer be reserved', async () => {
    ...
    await expect(
      service.reserveTrackedInventoryForOrder([
        {
          menuItemId: 'item_1',
          quantity: 2,
  ...
  it('allocates tracked menu item stock across inventory lots using FEFO ordering', async () => {
    ...
    expect(repository.decrementItemInventoryLotQuantity).toHaveBeenNthCalledWith(
      1,
      'lot_early',
      1,
      undefined,
  ...
  it('restores only tracked item and option inventory snapshots', async () => {
    ...
    expect(repository.incrementItemStock).toHaveBeenCalledWith(
      'item_1',
      1,
      undefined,
    );
    expect(repository.incrementItemInventoryLotRemainingQuantity).toHaveBeenCalledWith(
      'lot_1',
      1,
      undefined,
    );
    expect(repository.incrementOptionStock).toHaveBeenCalledTimes(1);
    expect(repository.incrementOptionStock).toHaveBeenCalledWith(
      'option_2',
      1,
      undefined,
    );
