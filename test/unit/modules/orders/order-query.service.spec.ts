...
describe('OrderQueryService', () => {
  const makeService = () => {
    ...
    const service = new OrderQueryService(repository, policy);
  ...
  it('lists customer orders by customer scope and attaches customer actions', async () => {
    ...
    jest
      ...
      .mockReturnValue(makeOrderSummary() as never);
    repository.findCustomerOrderSummaries.mockResolvedValue([{} as never]);

    const result = await service.listCustomerOrders(currentUser);

    expect(repository.findCustomerOrderSummaries).toHaveBeenCalledWith(
      'cust_prof_1',
    );
  ...
  it('lists merchant orders by merchant scope and attaches merchant actions', async () => {
    ...
    jest
      .spyOn(service, 'buildOrderSummary')
      .mockReturnValue(makeOrderSummary() as never);
    repository.findMerchantOrderSummaries.mockResolvedValue([{} as never]);
  ...
  it('returns admin order detail with admin override action attached', async () => {
    ...
    jest
      ...
      .mockReturnValue(makeOrderDetail() as never);
    repository.findOrderDetailById.mockResolvedValue({} as never);
  ...
  it('returns only mark_preparing for merchant order details that are already accepted', async () => {
    ...
    const { repository, service } = makeService();
    jest.spyOn(service, 'buildOrderDetail').mockReturnValue(
      makeOrderDetail({
        status: OrderStatus.MERCHANT_ACCEPTED,
      }) as never,
    );
    repository.findMerchantOrderDetail.mockResolvedValue({} as never);
  ...
  it('returns rider assignment actions for rider order details that are awaiting acceptance', async () => {
    ...
    repository.findRiderOrderDetail.mockResolvedValue({} as never);
  ...
  it('throws not found when a scoped order detail cannot be resolved', async () => {
    ...
    await expect(
      service.getCustomerOrderDetail(currentUser, 'order_missing'),
