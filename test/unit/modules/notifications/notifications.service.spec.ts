...
describe('NotificationsService', () => {
  ...
  it('builds a cursor-paginated notification page for inventory presets', async () => {
    ...
    await expect(
      service.listUserNotificationPage('merchant-user-1', {
        limit: 2,
  ...
  it('filters merchant inventory notifications by resolved history and exposes inventory snapshot metadata', async () => {
    ...
    await expect(
      service.listUserNotifications('merchant-user-1', {
        type: NotificationType.SYSTEM_ALERT,
  ...
  it('builds notification presets from unread facets', async () => {
    ...
    await expect(
      service.listNotificationPresets('merchant-user-1'),
    ).resolves.toEqual(
  ...
  it('bulk marks selected inventory alerts as read', async () => {
    ...
    await expect(
      service.bulkMarkInventoryAlertsRead('merchant-user-1', {
    ...
    expect(notificationDeliveryService.emitNotificationBulkRead).toHaveBeenCalledWith(
      'merchant-user-1',
      expect.objectContaining({
        markedCount: 1,
      }),
    ...
    expect(
      notificationDeliveryService.emitUnreadCountUpdated,
    ).toHaveBeenCalledWith('merchant-user-1', {
      unreadCount: 1,
    });
    expect(
      notificationDeliveryService.emitUnreadFacetsUpdated,
    ...
    expect(
      notificationDeliveryService.emitNotificationPresetsUpdated,
    ).toHaveBeenCalledWith(
      'merchant-user-1',
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it('creates a notification and maps it to the center entity', async () => {
    ...
    expect(notificationDeliveryService.emitNotificationCreated).toHaveBeenCalledWith(
      expect.objectContaining({
        notificationId: 'notification-2',
      }),
    );
  ...
  it('detects a recent duplicate merchant inventory alert by signature', async () => {
    ...
    await expect(
      service.hasRecentMerchantInventoryAlert({
        userId: 'user-3',
        resourceType: 'MENU_ITEM',
        resourceId: 'item_1',
        attentionLevel: 'LOW_STOCK',
  ...
  it('marks a notification as read when it exists', async () => {
    ...
    await expect(
      service.markNotificationRead('user-3', 'notification-3'),
    ).resolves.toMatchObject({
      notificationId: 'notification-3',
      readAt: '2026-04-23T12:00:00.000Z',
      inventoryAlert: null,
    });
    expect(notificationDeliveryService.emitNotificationRead).toHaveBeenCalledWith(
      expect.objectContaining({
        notificationId: 'notification-3',
      }),
    );
  ...
  it('creates a notification delivery attempt', async () => {
    repository.createDeliveryAttempt.mockResolvedValue({ id: 'attempt-1' } as never);
