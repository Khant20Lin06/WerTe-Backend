...
const admin_orders_controller_1 = require("../../../../src/modules/orders/controllers/admin-orders.controller");
...
describe('AdminOrdersController', () => {
    ...
    it('delegates list requests to the admin order query service', async () => {
        ...
        const controller = new admin_orders_controller_1.AdminOrdersController(orderQueryService, {});
    ...
    it('delegates detail requests to the admin order query service', async () => {
        ...
        expect(orderQueryService.getAdminOrderDetail).toHaveBeenCalledWith(currentUser, 'order_1');
    ...
    it('delegates admin cancellation to the admin order operations service', async () => {
        ...
        const controller = new admin_orders_controller_1.AdminOrdersController({}, adminOrderOperationsService);
        ...
        expect(adminOrderOperationsService.cancelAdminOrder).toHaveBeenCalledWith(currentUser, {
            orderId: 'order_1',
            reasonCode: 'admin_cancelled_duplicate_order',
            note: 'Customer placed a duplicate order.',
        });
    ...
    it('delegates admin status overrides to the admin order operations service', async () => {
        const adminOrderOperationsService = {
            overrideAdminOrderStatus: jest.fn().mockResolvedValue(makeOrderDetail({
        ...
        expect(adminOrderOperationsService.overrideAdminOrderStatus).toHaveBeenCalledWith(currentUser, {
            orderId: 'order_1',
            status: client_1.OrderStatus.RIDER_ASSIGNED,
            reasonCode: 'admin_override_manual_assignment',
            note: 'Dispatcher is moving the order into rider assignment.',
...
//# sourceMappingURL=admin-orders.controller.spec.js.map