...
let MerchantBranchesService = class MerchantBranchesService {
    constructor(prisma, merchantAccountService, branchesRepository, branchesService, branchPolicyService, zonesService) {
        this.prisma = prisma;
    ...
    async createCurrentMerchantBranch(currentUser, payload) {
        ...
        const storeType = this.normalizeStoreTypeCode(payload.storeType ?? merchant.storeType);
        ...
        const branch = await this.prisma.runInTransaction(async (tx) => {
            const createdBranch = await this.branchesRepository.create({
                ...
                ...(payload.operatingHours !== undefined
                    ? { operatingHours: (payload.operatingHours ?? client_1.Prisma.JsonNull) }
                    : {}),
            }, tx);
            if (zoneIds.length > 0) {
    ...
    async updateCurrentMerchantBranch(currentUser, branchId, payload) {
        ...
        const storeType = payload.storeType !== undefined
