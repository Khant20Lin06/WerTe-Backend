...
let MerchantAccountService = class MerchantAccountService {
    ...
    async updateCurrentMerchantProfile(currentUser, payload) {
        ...
        await this.merchantsService.invalidateCache(updatedMerchant.id, updatedMerchant.user.id);
    ...
    async resolveOwnedMerchant(currentUser) {
        ...
        if (merchant.status === client_1.MerchantStatus.PENDING) {
            throw new app_exception_1.AppException('Your merchant account is pending admin approval.', common_1.HttpStatus.FORBIDDEN, { code: error_codes_1.ErrorCodes.accountPending });
        }
        if (merchant.status === client_1.MerchantStatus.SUSPENDED) {
            throw new app_exception_1.AppException('Your merchant account has been suspended.', common_1.HttpStatus.FORBIDDEN, { code: error_codes_1.ErrorCodes.accountSuspended });
        }
    ...
    normalizeStoreTypeCode(storeType) {
