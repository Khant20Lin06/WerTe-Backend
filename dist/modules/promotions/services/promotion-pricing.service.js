...
let PromotionPricingService = class PromotionPricingService {
    ...
    async evaluatePromotionForCheckout(input) {
        ...
        if (promotion.perCustomerLimit !== null &&
            ...
            const usageCount = await this.promotionsRepository.countCustomerUsage(promotion.id, input.customerProfileId);
            if (usageCount >= promotion.perCustomerLimit) {
                throw new app_exception_1.AppException('You have already used this promotion the maximum number of times.', common_1.HttpStatus.UNPROCESSABLE_ENTITY, {
                    code: error_codes_1.ErrorCodes.unprocessableEntity,
                    details: {
                        promotionId: promotion.id,
        ...
        if (promotion.discountType === client_1.PromotionDiscountType.FREE_DELIVERY) {
            ...
            discountAmount = client_1.Prisma.Decimal.min(cappedDiscount, input.subtotalAmount);
        }
        return {
            promotionId: promotion.id,
            code: promotion.code,
