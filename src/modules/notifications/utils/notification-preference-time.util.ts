...
export function parseLocalMinuteOfDay(value: string): number {
  const [hour, minute] = value.split(':');

  return Number(hour) * 60 + Number(minute);
}

export function resolveLocalMinuteOfDay(at: Date, timeZone: string): number {
...
export function resolveLocalSecondOfDay(at: Date, timeZone: string): number {
  const formatter = new Intl.DateTimeFormat('en-US', {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
...
export function isInventoryAlertPushMutedNow(
  ...
  const currentMinute = resolveLocalMinuteOfDay(
    at,
    preference.inventoryAlertQuietHoursTimezone,
  );
  const startMinute = parseLocalMinuteOfDay(
    preference.inventoryAlertQuietHoursStartLocalTime,
  );
  const endMinute = parseLocalMinuteOfDay(
    preference.inventoryAlertQuietHoursEndLocalTime,
  );
...
export function resolveNextQuietHoursBoundaryAt(
  preference: MerchantInventoryAlertPreferenceEntity,
  from = new Date(),
): Date | null {
  if (
    !preference.inventoryAlertPushEnabled ||
    !preference.inventoryAlertQuietHoursEnabled ||
    preference.inventoryAlertQuietHoursStartLocalTime === null ||
    preference.inventoryAlertQuietHoursEndLocalTime === null ||
    preference.inventoryAlertQuietHoursTimezone === null
  ...
  const currentSecondOfDay = resolveLocalSecondOfDay(
    from,
    preference.inventoryAlertQuietHoursTimezone,
  );
  ...
  const nextBoundaryDelaySeconds = Math.min(
    ...boundarySeconds.map((boundarySecond) => {
