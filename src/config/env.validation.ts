...
export const envValidationSchema = Joi.object({
  ...
  S3_ACCESS_KEY_ID: Joi.string().trim().allow('').optional(),
  S3_SECRET_ACCESS_KEY: Joi.string().trim().allow('').optional(),
  S3_PUBLIC_BASE_URL: Joi.string().uri().allow('').optional(),
  UPLOAD_MAX_FILE_SIZE_BYTES: Joi.number().default(10 * 1024 * 1024),
  UPLOAD_ALLOWED_MIME_TYPES: Joi.string().trim().default('image/jpeg,image/png,image/webp'),
